...
let ErrorCount = new Counter("a_errors");
let SuccessCount = new Counter("a_successes");
let ErrorRate = new Rate("a_error_rate");
...
export let options = {
	...
	thresholds: {
		a_error_rate: ['rate<0.1'], // <10% errors
	}
};
...
export default function() {
	...
	group("page_1 - https://jan.sevela.cz/", function() {
		...
		res.forEach(function(response) {
			success = check(response, { // use response here
				'Status is OK': (r)=> r.status < 399,
			});
			if (!success) {
				ErrorCount.add(1);
				ErrorRate.add(true);
